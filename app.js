...
const BASE_DAILY_HEARTS = 3; // reset to 3 daily
...
/* Stats (kept for the results card) */
...
function finishRun(perfect) {
  ...
  renderHUD();
  renderResults(perfect); // keeps stats data fresh
  renderHomeState(); // updates dashboard & countdown
  openResultsModal(daily.lastResults); // NYT-style summary modal
}
...
function renderResults(perfect) {
  ...
  // do not switch to stats here; we use the modal instead
}
...
function renderHomeTodayCard(snapshot) {
  ...
  const total = snapshot.results?.length || 6;
...
function tickCountdown() {
  ...
  function step() {
    ...
    if (ms <= 0) {
      ...
      const tl = new Date().toISOString().split("T")[0];
      if (profile.lastPlayLocal !== tl || profile.hearts < BASE_DAILY_HEARTS) {
        profile.hearts = BASE_DAILY_HEARTS;
        profile.lastPlayLocal = tl;
        saveProfile(profile);
        renderHUD();
      }
      enforceDailyLock();
...
function renderCatStats() {
  ...
  // Sort: most answered first; tie-break by best accuracy
...
function openResultsModal(snapshot) {
  ...
  const total = snapshot.results?.length || session.questions.length || 6;
