...
const linkX = $("#share-x");
const linkWA = $("#share-wa");
const linkTG = $("#share-tg");
const linkFB = $("#share-fb");
const btnSMS = $("#share-sms");
const btnCopy = $("#share-copy");

const startBtn = $("#start-game"); // [NEW]
const lockMsg = $("#lock-msg"); // [NEW]
const countdownEl = $("#countdown"); // [NEW]
...
// ---- Wordle-style daily lock state [NEW] ----
...
function tickCountdown() {
  ...
  function step() {
    countdownEl.textContent = fmt(msUntilTomorrowUTC());
...
function buildDailyQuestions() {
  ...
  const mm = nowUtc.getUTCMonth() + 1;
  const dd = nowUtc.getUTCDate();
  const dayKey = `${yyyy}-${mm}-${dd}`;
  return CATS.map((cat) => {
    const pool = BANK.filter((q) => q.category === cat);
    if (!pool.length) return null;
    const offset = hashStr(`dt-v1|${dayKey}|${cat}`) % pool.length;
    return pool[offset];
  }).filter(Boolean);
}
...
function renderResults(perfect) {
  ...
  resXPBreak.textContent = `+${base} (answers) +${finish} (finish)${
  ...
  const snapshot =
    session.finished && session.results.length
      ? {
          ...
          xpToday: session.xpToday,
          hearts: session.hearts,
          perfect: session.correctCount === (session.questions?.length || 5),
        }
