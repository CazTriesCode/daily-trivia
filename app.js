/* =========================================================
   Daily Trivia — Wordle-style daily (UTC) with stats & share
   ========================================================= */
...
// Deterministic daily seed salt (shared set per UTC day)
const PUBLIC_SALT = "dt-v1";

// ---- Question bank (add more per category for variety) ----
const BANK = [
  {
    category: "animals",
    q: "Which mammal has the most powerful bite?",
    a: "Hippopotamus",
    distractors: ["Tiger", "Grizzly bear", "Polar bear"],
  },
  {
    category: "animals",
    q: "Which sea creature has the largest eyes?",
    a: "Giant squid",
    distractors: ["Blue whale", "Manta ray", "Oarfish"],
  },

  {
    category: "gaming",
    q: "OSRS: what skill is trained by chopping trees?",
    a: "Woodcutting",
    distractors: ["Fletching", "Construction", "Firemaking"],
  },
  {
    category: "gaming",
    q: "Destiny 2: the main social hub is…",
    a: "The Tower",
    distractors: ["The Farm", "The City", "The Reef"],
  },

  {
    ...
    a: "Mjolnir",
    distractors: ["Gungnir", "Stormbreaker", "Hofund"],
  },
  {
    category: "marvel",
    q: "Who is the Winter Soldier?",
    a: "Bucky Barnes",
    distractors: ["Sam Wilson", "Steve Rogers", "James Rhodes"],
  },

  {
    category: "lotr",
    q: "Who destroys the One Ring (movies)?",
    a: "Frodo and Gollum",
    distractors: ["Samwise", "Gandalf", "Aragorn"],
  },
  {
    category: "lotr",
    q: "Frodo’s glowing sword is…",
    a: "Sting",
    distractors: ["Glamdring", "Andúril", "Narsil"],
  },

  {
    category: "cars",
    q: "Which brand makes the Mustang?",
    a: "Ford",
    distractors: ["Chevrolet", "Dodge", "Toyota"],
  },
  {
    category: "cars",
...

  {
    ...
    a: "Ravenclaw",
    distractors: ["Hufflepuff", "Gryffindor", "Slytherin"],
  },
  {
    category: "hp",
    q: "What position does Harry play in Quidditch?",
    a: "Seeker",
    distractors: ["Chaser", "Beater", "Keeper"],
  },
  {
    category: "hp",
    q: "What is the spell to disarm an opponent?",
    a: "Expelliarmus",
    distractors: ["Stupefy", "Petrificus Totalus", "Lumos"],
  },
];
...
const resultScoreTotal = $("#result-total"); // <span id="result-total"> in HTML (denominator)
const backHomeBtn = $("#back-home");
const shareResultsBtn = $("#share-results"); // single Share button

// Home lock bits
const startBtn = $("#start-game");
const lockMsg = $("#lock-msg"); // e.g., "You've played today..."
const countdownEl = $("#countdown"); // shows UTC countdown
const homeLocked = $("#home-locked"); // card with stats/collection on Home
const catStatsEl = $("#cat-stats"); // grid for per-category stats
const miniCollectionEl = $("#mini-collection");

// ---- Persistence (profile) ----
const KEY = "dt_profile_v5";
...
function buildDailyQuestions() {
  const now = new Date();
  const dayKey = `${now.getUTCFullYear()}-${
    now.getUTCMonth() + 1
  }-${now.getUTCDate()}`;
  return CATS.map((cat) => {
...
function onAnswer(q, choice, clicked) {
  ...
  // visual feedback
  $$("#answer-options button").forEach((b) => {
...
function renderResults(perfect) {
  ...
  // dynamic denominator (no optional chaining on assignment)
  ...
  if (resStreak) resStreak.textContent = `${profile.streak} 🔥`;
  ...
  if (resRewardRow) {
    if (perfect) {
      ...
      if (resRewardImg) resRewardImg.src = rewardImg?.src || "";
    } else {
...
function maybeAwardSticker() {
  ...
  const idx = Math.floor(Math.random() * 4) + 1; // sticker_1..sticker_4.png
...
function renderHomeLocked() {
  if (!homeLocked) return;

  // Per-category lifetime stats
  if (catStatsEl) {
    ...
    CATS.forEach((cat) => {
      ...
      tile.innerHTML = `
           <div class="muted">${LABELS[cat]}</div>
           <div class="big">${acc}%</div>
           <div class="small">${st.correct}/${st.total} correct</div>
         `;
      catStatsEl.appendChild(tile);
    });
  }

  // Mini collection (last 8)
  if (miniCollectionEl) {
    miniCollectionEl.innerHTML = "";
    (profile.inventory || []).slice(-8).forEach((src) => {
      const img = document.createElement("img");
      img.src = src;
      img.alt = "Sticker";
      img.className = "thumb";
      miniCollectionEl.appendChild(img);
    });
  }

  homeLocked.classList.remove("hidden");
}
...
function handleShare() {
  // If viewing saved stats without an active session, fall back to snapshot
  const total =
    session.questions?.length || daily.lastResults?.results?.length || 6;
  const correct =
    typeof session.correctCount === "number"
      ? session.correctCount
      : daily.lastResults?.correctCount || 0;

  const text = `Daily Trivia — ${correct}/${total} correct!
...
function enforceDailyLock() {
  ...
  if (played) {
    ...
    homeLocked?.classList.add("hidden");
  }
}
...
backHomeBtn?.addEventListener("click", () => {
  switchScreen("home");
  enforceDailyLock(); // <- ensures Start hides & the lock card shows
});
...
document.getElementById("open-stats")?.addEventListener("click", () => {
  const snapshot =
    session.finished && session.results.length
      ? {
          results: session.results,
          correctCount: session.correctCount,
          xpToday: session.xpToday,
          hearts: session.hearts,
          perfect: session.correctCount === session.questions.length,
        }
  ...
  // Hydrate session-like view to reuse renderResults
